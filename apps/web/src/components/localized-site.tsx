...
const copy = {
  en: {
    ...
    tickerItems: ["Growth Strategy", "Customer Intelligence", "Conversion Design", "E-commerce", "Web Development", "Dashboards & Automation", "CRO", "Measurement"],
    whyEyebrow: "Why us",
    whyTitle: "Built to be the partner that actually ships.",
    whys: [
      { icon: "◉", title: "Intelligence That Leads to Action", text: "Insight that ends in a decision — not another deck of findings." },
      { icon: "↗", title: "Strategy Connected to Delivery", text: "The same senior people who plan are the ones who build." },
      { icon: "◆", title: "Measurement Built In", text: "Every change ships with a way to see whether it worked." },
      { icon: "▲", title: "Senior, Small, Direct", text: "Your problem gets direct attention — no junior hand-offs." },
    ],
  ...
  id: {
    ...
    tickerItems: ["Strategi Pertumbuhan", "Intelijen Pelanggan", "Desain Konversi", "E-commerce", "Pengembangan Web", "Dashboard & Automasi", "CRO", "Pengukuran"],
    whyEyebrow: "Kenapa kami",
    whyTitle: "Dibangun untuk jadi partner yang benar-benar menyelesaikan.",
    whys: [
      { icon: "◉", title: "Intelijen yang Berujung Aksi", text: "Wawasan yang berakhir pada keputusan — bukan deck temuan lain." },
      { icon: "↗", title: "Strategi Tersambung ke Eksekusi", text: "Orang senior yang merencanakan adalah yang membangun." },
      { icon: "◆", title: "Pengukuran Bawaan", text: "Setiap perubahan dilengkapi cara untuk melihat apakah berhasil." },
      { icon: "▲", title: "Senior, Kecil, Langsung", text: "Masalahmu mendapat perhatian langsung — tanpa serah-terima junior." },
    ],
...
export function LocalizedHome({ locale }: { locale: Locale }) {
  const t = copy[locale];
  return <><section className="hero commercial-hero"><div className="hero-inner"><div className="hero-split"><div className="hero-left"><div className="hero-label"><span>{t.heroLabel}</span><span>Indonesia / Remote</span></div><h1>{t.heroTitle}</h1><p className="hero-text">{t.heroText}</p><p className="hero-proof">{t.heroProof}</p><div className="hero-actions"><Link className="button button-accent" href={localizedPath(locale, "/audit")}>{t.heroCtaPrimary} <span>→</span></Link><a className="text-link" href="#problems">{t.heroCtaSecondary} <span>↓</span></a></div></div><div className="hero-right"><div className="hero-visual" aria-hidden="true"><div className="hero-window"><span /><span /><span /><em>flective.vercel.app</em></div><div className="hero-dash"><div className="hero-side"><i /><i /><i /><i /></div><div className="hero-main"><div className="hero-kpis"><div><span>Traffic</span><svg viewBox="0 0 80 24" preserveAspectRatio="none"><polyline points="0,18 16,14 32,16 48,9 64,11 80,4" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg></div><div><span>Conversion</span><svg viewBox="0 0 80 24" preserveAspectRatio="none"><polyline points="0,20 16,18 32,12 48,14 64,8 80,6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg></div><div><span>Signal</span><svg viewBox="0 0 80 24" preserveAspectRatio="none"><polyline points="0,10 16,14 32,8 48,12 64,6 80,9" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg></div></div><div className="hero-chart"><div className="hero-chart-grid"><svg viewBox="0 0 600 220" preserveAspectRatio="none"><line x1="0" y1="0" x2="600" y2="0" /><line x1="0" y1="55" x2="600" y2="55" /><line x1="0" y1="110" x2="600" y2="110" /><line x1="0" y1="165" x2="600" y2="165" /><line x1="0" y1="220" x2="600" y2="220" /><path d="M0,190 C60,170 90,120 150,130 C210,140 240,80 300,90 C360,100 390,50 450,60 C510,70 560,30 600,24 L600,220 L0,220 Z" fill="color-mix(in srgb, var(--acid) 14%, transparent)" stroke="none" /><path d="M0,190 C60,170 90,120 150,130 C210,140 240,80 300,90 C360,100 390,50 450,60 C510,70 560,30 600,24" className="chart-line" fill="none" stroke="var(--acid)" strokeWidth="3" strokeLinecap="round" /></svg></div><div className="hero-chart-meta"><span>+ Growth signal</span><span>Live · sample data</span></div></div></div></div></div></div><div className="hero-stats">{t.heroStats.map((s, i) => <div className={`hero-statcard${i === 0 ? " dark" : ""}${i === 1 ? " accent" : ""}`} key={s.label}><strong>{s.value}</strong><span>{s.label}</span><small>{s.note}</small></div>)}</div></div></div></section><div className="ticker" aria-hidden="true"><div className="ticker-track">{[...t.tickerItems, ...t.tickerItems].map((item, i) => <span key={i}>{item}<i>•</i></span>)}</div></div><section id="problems" className="section"><div className="shell section-intro"><p className="eyebrow">{t.problemsEyebrow}</p><div><h2 className="section-heading">{t.problemsTitle}</h2><p className="section-lede">{t.problemsLead}</p></div></div><div className="shell friction-list">{t.problems.map((item, i) => <div key={item}><span>0{i + 1}</span><p>{item}</p></div>)}</div></section><section className="section offer-section"><div className="shell offer-layout"><div><p className="eyebrow">{t.offerEyebrow}</p><h2 className="section-heading">{t.offerTitle}</h2><p className="section-lede">{t.offerLead}</p><Link className="text-link" href={localizedPath(locale, "/audit")}>{t.offerLink} <span>→</span></Link></div><div className="offer-card"><div className="offer-card-top"><span>FLECTIVE / AUDIT</span><span>01</span></div><h3>{t.offerCard}</h3><ul>{t.outcomes.map((x) => <li key={x}><span>+</span>{x}</li>)}</ul><Link className="button" href={localizedPath(locale, "/audit")}>{t.cta} <span>→</span></Link></div></div></section><section className="section system-section"><div className="shell"><p className="eyebrow">{t.methodEyebrow}</p><h2 className="section-heading">{t.methodTitle}</h2><div className="loop">{t.method.map(([n, title, text]) => <div className="loop-step" key={title}><span>{n}</span><strong>{title}</strong><p>{text}</p></div>)}</div></div></section><section className="section"><div className="shell"><p className="eyebrow">{t.whyEyebrow}</p><h2 className="section-heading">{t.whyTitle}</h2><div className="why-grid">{t.whys.map((w) => <article key={w.title}><div className="why-icon">{w.icon}</div><h3>{w.title}</h3><p>{w.text}</p></article>)}</div></div></section><section className="section section-line"><div className="shell"><p className="eyebrow">{t.capabilitiesEyebrow}</p><h2 className="section-heading">{t.capabilitiesTitle}</h2><div className="capability-grid">{services.map((service, index) => <article className="capability" key={service.slug}><span className="number">0{index + 1} / {service.eyebrow.split("—")[1]?.trim()}</span><h3>{service.name}</h3><p>{service.statement}</p><ArrowLink href={`/${locale}/services/${service.slug}`}>{t.explore}</ArrowLink></article>)}</div></div></section><section className="section"><div className="shell"><p className="eyebrow">{t.workEyebrow}</p><h2 className="section-heading">{t.workTitle}</h2><p className="section-lede section-lede-narrow">{t.workLead}</p><div className="work-grid">{caseStudies.map((study) => <Link href={`/${locale}/work/${study.slug}`} className={`work-card ${study.palette}`} key={study.slug}><div className="work-card-meta"><span>{study.client}</span><span>{study.year}</span></div><h3>{study.project}</h3><p>{study.summary}</p><span className="arrow-link">{t.seeWork} <span>↗</span></span></Link>)}</div><div className="section-bridge"><p>Want to be the next useful case?</p><Link className="text-link" href={localizedPath(locale, "/contact")}>{locale === "id" ? "Ceritakan apa yang macet" : "Tell us what is stuck"} <span>→</span></Link></div></div></section><section className="section"><div className="shell"><p className="eyebrow">{t.processEyebrow}</p><h2 className="section-heading">{t.processTitle}</h2><div className="process-list">{t.process.map(([number, title, text]) => <article className="process-item" key={number}><span>{number}</span><h3>{title}</h3><p>{text}</p></article>)}</div></div></section><section className="section faq-section"><div className="shell"><div className="faq-head"><p className="eyebrow">{t.faqEyebrow}</p><div><h2 className="section-heading">{t.faqTitle}</h2><p className="section-lede">{t.faqLead}</p></div></div><div className="faq-list">{t.faqs.map((item) => <details key={item.q}><summary>{item.q}<span>+</span></summary><p>{item.a}</p></details>)}</div></div></section><section className="section"><div className="shell"><section className="cta-section"><p className="eyebrow">{t.finalEyebrow}</p><h2>{t.finalTitle}</h2><p>{t.finalText}</p><Link className="button" href={localizedPath(locale, "/audit")}>{t.heroCtaPrimary} <span>→</span></Link></section></div></section></>;
}
